...
- **PDF to Markdown 변환기** (`pdf-to-markdown`) v1.0 — 최초 등록 (PRODUCTIVITY, 영문 전용 — 한국어판 없음)
- **PDF to Markdown 변환기** (`pdf-to-markdown`) v1.1 — 한국어판 신규 추가 — 그동안 영문 전용이었던 도구에 한국어판 제작. 안내문(표 감지 알고리즘, AI 정밀정리 등 상세 설명 포함)을 온전히 번역해 이식. 영문판의 가짜 사이드바 링크(실제 존재하지 않는 도구명)도 정정
