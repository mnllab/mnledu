...
- **스마트 회의록** (`meeting-minutes`) v1.2 — 디자인 개선 — 미리보기·내보내기 패널을 오른쪽에서 슬라이드되어 나오는 드로어로 변경해 입력창을 항상 전체 폭으로 씀. 질의응답 상태를 3단계 드롭다운에서 '답변 완료' 체크박스로 단순화(체크 시 카드 배경이 옅어짐). 답변란은 기본적으로 접혀 있고 '+ 답변 추가'를 눌러야 나타나는 아코디언 방식으로 변경
- **스마트 회의록** (`meeting-minutes`) v1.3 — '답변 완료' 체크박스를 없애고, 답변란에 글자를 적으면 자동으로 완료 상태로 인식하도록 단순화
